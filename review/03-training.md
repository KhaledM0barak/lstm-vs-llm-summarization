...
were found by *measuring* rather than by reading code, and one of them was a 34-55x
speedup.
...
tensor: at batch 64 x 100 steps that's **1.28 GB in fp32**, and the original
...
almost every step, so training spent most of its wall-clock in shader
compilation, with `MTLCompilerService` pinning two CPU cores, instead of
arithmetic. Rounding
...
|---|---|---|---|
| Bahdanau + input feeding | 2537 s | 73.7 s | **34x** |
