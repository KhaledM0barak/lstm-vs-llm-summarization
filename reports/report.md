...
are not redistributed. We also cannot verify what was in the LLM's pretraining
data, so its numbers should be read as an upper bound on genuinely held-out
performance; our few-shot exemplars come from train, which controls the leakage
we can control. On compute, training took 8.73 GPU-hours across all four runs
(3.05 for the model itself) plus 4.07 of generation: small in absolute terms, but
the LLM's pretraining cost is amortized across all users and invisible here.
