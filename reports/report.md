...
**Code.** The implementation in this repository was written with substantial
assistance from Claude (Anthropic), used as an interactive coding assistant. This
includes the data pipeline, the model, the training loop, the LLM baseline
harness, and the evaluation and analysis scripts. The team reviewed and tested
the generated code, ran the full pipeline end to end, and is responsible for its
correctness; each member owns a specific component and can account for its
design decisions (see Appendix B).

**Results.** No reported number was written by a language model. Every figure in
this report is produced by executing the committed code and is regenerated by
`python scripts/collect_results.py`, which reads the run artifacts directly and
refuses to emit a number whose artifact is missing. The results are therefore
reproducible from the repository by anyone.

**Report.** This report was drafted with Claude and edited by the team. The
analytical claims were checked against the measured diagnostics rather than
asserted from the literature — for example, the repetition and
fixed-vector-bottleneck claims in §4.3 and §5 are supported by the
duplicate-trigram and unsupported-content rates in the results tables.
